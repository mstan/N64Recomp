...
- **Handles the parts of the game that are packed away and unpacked while
  it runs.** Stadium keeps much of itself compressed and only expands
  pieces on demand. This unpacks them when the port is built, recognizes
  the repeating ones, and arranges them in memory so they no longer
  overwrite each other (which had been corrupting data).
- **Fixes pointers in that unpacked code** so they still point to the
  right place once a piece is loaded into memory.
- **Corrects a handful of individual CPU instructions** the tool had been
  translating slightly wrong — small errors that led to subtle misbehavior
  or crashes.
- **Fixes crashes and a quit-to-menu freeze** caused by how the game, and
  especially its built-in Game Boy emulator, chains function calls inside
  very tight loops.
- **Translates the audio/graphics co-processor code more faithfully, and
  adds a side-by-side comparison harness against the ares emulator** that
  was used to pin down exactly where the PC version drifted from the real
  console.
...
Recompiler output can be compiled with any C compiler (tested with msvc, gcc and clang). The output is expected to be used with a runtime that can provide the necessary functionality and macro implementations to run it. A runtime is provided in [N64ModernRuntime](https://github.com/mstan/N64ModernRuntime).
...
This saves a tremendous amount of time while iterating on patches for the target binary, as you can bypass rerunning the recompiler on the target binary as well as compiling the original recompiler output.
